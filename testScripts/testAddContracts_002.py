...
class AddContract(unittest.TestCase):
    ...
    def test_add_contract(self):
        ...
        try:
            ...
            # 获取账号是否执行
            is_execute_user = self.excelObj.get_column(user_sheet, account_isExecute)
            # 获取账号sheet中的数据表列
            data_book_column = self.excelObj.get_column(user_sheet, account_dataBook)
            for idx, i in enumerate(is_execute_user[1:]):
                ...
                if i == "y":   # 表示要执行
                    ...
                    try:
                        ...
                        self.excelObj.write_cell(user_sheet, 'failed',
                                                 row_number=idx + 2,
                    ...
                    # 获取联系人数据表中联系人是否执行
                    is_execute_data = self.excelObj.get_column(data_sheet, contracts_isExecute)
                    ...
                    for id, data in enumerate(is_execute_data[1:]):
                        ...
                        if data == 'y':
                            ...
                            # 在联系人工作表写入添加联系人执行时间
                            self.excelObj.write_cell_currenttime(data_sheet,
                                                                 row_number=id+2,
                            ...
                            try:
                                ...
                                self.excelObj.write_cell(data_sheet, 'failed',
                                                         row_number=id+2,
                                ...
                                # 断言成功，写入添加联系人信息成功
                                self.excelObj.write_cell(data_sheet, 'pass',
                                                         row_number=id+2,
                                                         column_number=contracts_testResult,
                                                         style='green')
                                contract_num += 1
                        ...
                        if contract_num == is_execute_num:
                            ...
                            self.excelObj.write_cell(user_sheet, 'pass',
                                                     row_number=idx + 2,
                                                     column_number=account_testResult,
                                                     style='green')
                    ...
                    logging.info(u'用户%s被设置为忽略执行' % self.excelObj.get_cell_value(user_sheet,
                                                                                row_number=idx+2,
